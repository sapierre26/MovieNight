...
router.post("/register", (req, res) => {
  const { username, password, image, name, hometown, bio } = req.body;
  if (typeof username !== "string" || typeof password !== "string" || typeof image !== "string" || typeof name !== "string" || typeof hometown !== "string" || typeof bio !== "string") {
    return res.status(400).send("Bad request: Invalid input data.");
  } else {
    import_credential_svc.default.create(username, password, image, name, hometown, bio).then((creds) => generateAccessToken(creds.username)).then((token) => {
